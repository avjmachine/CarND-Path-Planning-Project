...
int main() {
  ...
  h.onMessage([&REF_SPEED, &current_speed, &lane_no, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
	  ...
	  if (change_lane) {
	      //finite state machine where lane changes can only happen one at a time
	      //if starting position is lane 1 or 2, normally the ego vehicle can shift to a lower lane
	      //else if already in lane 0, can shift to a higher lane 1
	      ...
	      // checking if lane change is possible
	      if (is_lane_free(sensor_fusion, current_ref_s, current_speed, to_lane, prev_path_size)) {
                    lane_no = to_lane;
		    std::cout<<"Lane changed to :"<<lane_no<<"\n";
	       }
	      // giving an exception to the ego vehicle to move from from lane 1 to 2 if lane 0 is not free and lane 2 is free
	      else if ((lane_no==1) && (is_lane_free(sensor_fusion, current_ref_s, current_speed, 2, prev_path_size))) {
	            lane_no = 2;
		    std::cout<<"Special Lane Change! Lane changed from 1 to 2!\n";
	      }
	      // if no lane change are possible, abort all lane changes and apply brakes
	      else {
                    change_lane = false;  
	            apply_brakes = true;
               }
	  ...
	  else if ((current_speed < REF_SPEED) && (!change_lane)) { 
	      current_speed += 0.25;
